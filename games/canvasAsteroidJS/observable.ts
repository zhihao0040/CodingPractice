...
class Observable<Input> {
  ...
  /**
   * Creates a child Observable that is detached when boolean condition evaluates to false
   * @param condition the boolean to indicate whether the child should be detached.
   * @return child Observable that stops firing when condition aren't met
   */
  takeWhile(condition: (_:Input)=>boolean):Observable<Input>{
    return new Observable<Input>(observer => {
      const oUnsub = this.subscribe(e=>{
        if (condition(e)) {
          observer.next(e);
        }
        else{
  ...
  /**
   * Creates a child Observable that only fires once after boolean condition valutes to true
   * @param condition the boolean to indicate whether it should fire
   * @return child Observable that fires only once when condition is true.
   */
  takeOnceWhen(condition: (_:Input)=>boolean):Observable<Input>{
    return new Observable<Input>(observer => {
      const oUnsub = this.subscribe(e=>{
        if (condition(e)) {
          observer.next(e)
